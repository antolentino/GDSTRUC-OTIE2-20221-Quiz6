...
public class SimpleHashtable {
    ...
    //QUIZ 4 - REMOVE() FUNCTION FOR HASHTABLE WITH LINEAR PROBING
    public void remove(String key) {
        ...
        if (!isOccupied(hashedKey)) {
            ...
            // search the array for the next position
            while (!isOccupied(hashedKey) && hashedKey != stoppingIndex) {
                hashedKey = (hashedKey + 1) % hashtable.length;
            }
        }
        ...
        if (!isOccupied(hashedKey)) {
            System.out.println("Sorry, there is no more element to remove. " +
                    "You have reached the stopping index.");
        } else {
